defmodule Numbers do
  ...
  koan "Another way to divide" do
    # Devuelve el cociente entero (sin decimales)
    assert div(5, 2) == 2
  end

  koan "What remains or: The Case of the Missing Modulo Operator (%)" do
    # Función que se utiliza para calcular el resto de una división entera
    assert rem(5, 2) == 1
  end
  ...
  koan "Two ways to round, are they exactly the same?" do
    # Float.round() - redondea al entero mas cercano, pero devuelve un número flotante
  ...
  koan "Let's grab the individual digits in a list" do
    # Se utiliza para descomponer un número entero en una lista de sus dígitos individuales
    individual_digits = Integer.digits(58_127)
    assert individual_digits == [5, 8, 1, 2, 7]
  end

  koan "Oh no! I need it back together" do
    # Dada una lista, devuelve un intero con los elementos de una lista
    number = Integer.undigits([1, 2, 3, 4])
    assert number == 1234
  end

  koan "Actually I want my number as a string" do
    #Convierte un entero a un cadena o string
  ...
  koan "What if you parse a floating point value as an integer?" do
    # Aquí solo tomamos la parte entera reconocida como número
    assert Integer.parse("1.2") == {1, ".2"}
  end

  koan "Just want to parse to a float" do
    # Analiza una representación de texto de un número flotante
  ...
  koan "I don't want this decimal point, let's round up" do
    # Función que se utiliza para redondear un número de punto flotante hacia arriba al entero más cercano
    assert Float.ceil(34.25) === 35.0
  end

  koan "OK, I only want it to 1 decimal place" do
                  # Numero decimales que desees
    assert Float.ceil(34.25, 1) === 34.3
  end

  koan "Rounding down is what I need" do
    # Redondea un flotante al número más grande menor o igual a num.
  ...
  koan "Round the number up or down for me" do
    ...
    # round(float, precision(numero de decimales))
